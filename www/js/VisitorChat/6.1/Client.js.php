import * as validatorModule from "<?php echo \UNL\VisitorChat\Controller::$url . 'wdn/templates_6.1/js/plugins/plugin.form-validator.js'?>";
import "<?php echo \UNL\VisitorChat\Controller::$url . 'js/VisitorChat/6.1/form.js'?>";
import { getCookie, setCookie } from "<?php echo \UNL\VisitorChat\Controller::$url . 'wdn/templates_6.1/js/lib/unl-utility.js'?>";

const $ = await validatorModule.initialize();
window.UCOMMCHAT.loadForms($);

const serverURLPHP = "<?php echo \UNL\VisitorChat\Controller::$url;?>";
const refreshRatePHP = "<?php echo \UNL\VisitorChat\Controller::$refreshRate;?>";
window.UCOMMCHAT.visitorChat = null;

<?php
    require_once(__DIR__ . "/ChatBase.js");
...
class VisitorChat_Client extends VisitorChat_ChatBase {
    loginHTML = false;
    clientName = false;
    ...
    setSessionCookie(name, value, expiresInSeconds, path) {
        setCookie(name, value, expiresInSeconds, path, null, 'none', true);
    };
    ...
    startEmail() {
        ...
        window.UCOMMCHAT.visitorChat.displayWelcomeMessage();

        if (window.UCOMMCHAT.visitorChat.operatorsAvailable) {
            ...
            $('#visitorChat_methods_chat').one('click', function() {
                window.UCOMMCHAT.visitorChat.stop(function(){
                    window.UCOMMCHAT.visitorChat.startChat();
            ...
            $('#visitorChat_methods_chat').one('click', function() {
                window.UCOMMCHAT.visitorChat.stop(function(){
                window.UCOMMCHAT.visitorChat.startChatBot();
                $('#visitorChat_messageBox').keyup();
                });
    ...
    startChat(chatInProgress) {
        ...
        $('#visitorChat_methods_email').one('click', function() {
            window.UCOMMCHAT.visitorChat.stop(function(){
                window.UCOMMCHAT.visitorChat.startEmail();
                $('#visitorChat_messageBox').keyup();
            });
            return false;
        });
    ...
    startChatBot(chatInProgress) {
        this.method = 'chatbot';
        this.displaySiteAvailability();
        this.launchChatContainer();
        //console.log('starting chatbot: ' + window.UCOMMCHAT.visitorChat.chatbotUserID);
        //console.log('starting chatbot: ' + window.UCOMMCHAT.visitorChat.getChatbotUserID());
        ...
        var testNotice = '';
        if (window.UCOMMCHAT.visitorChat.chatbotEnv != 'PROD') {
        testNotice = ' (Test Env)';
        }
    ...
    startChatBotWithIntent(introMsg, intentMsg, intentSessionAttributes, displayChatMethods) {
        // Default parameters
        ...
        //console.log('starting chatbot intent: ' + window.UCOMMCHAT.visitorChat.chatbotUserID);
        //console.log('starting chatbot: ' + window.UCOMMCHAT.visitorChat.getChatbotUserID())
        if (window.UCOMMCHAT.visitorChat.operatorsAvailable || !this.isChatbotAvailable() || introMsg.trim().length == 0 || intentMsg.trim().length == 0) {
    ...
    onOperatorMessage(message) {
        ...
        if (!getCookie('UNL_Visitorchat_FirstOperatorResponse') && getCookie('UNL_Visitorchat_Start')) {
    ...
    launchChatContainer() {
        ...
        $('#visitorChat_container').delay(10).slideDown(320, function() {
            if (window.UNL.idm.getDisplayName()) {
                $('#visitorChat_name').val(window.UNL.idm.getDisplayName());
            }
        });
    }
    ...
    ajaxBeforeSubmit(arr, $form, options) {
        ...
        return super.ajaxBeforeSubmit(arr, $form, options);
    }
    ...
    initWatchers() {
        ...
        $('#visitorChat_confirmationEmailForm').bind('validate-form', function (event, result) {
            if (result) {
                ...
                $('#visitorChat_sendAnotherConfirmation').click(function(){
                    $('#visitorChat_confirmationContainer').html($(window.UCOMMCHAT.visitorChat.confirmationHTML).filter('#visitorChat_confirmationContainer').html()).focus();
                    window.UCOMMCHAT.visitorChat.initWatchers();
                    return false;
                });
            }
        ...
        $('#visitorChat_header, #dcf-mobile-toggle-chat').on('click keypress', function (event) {
            ...
            if (!$('#visitorChat_container').is(':visible')) {
                //Open the container
                window.UCOMMCHAT.visitorChat.widgetIsOpen = true;
                $('#visitorChat').addClass('visitorChat_open');
                ...
                //Close the container
                window.UCOMMCHAT.visitorChat.widgetIsOpen = false;
                ...
                if (window.UCOMMCHAT.visitorChat.chatStatus == "LOGIN") {
                    ...
                    window.UCOMMCHAT.visitorChat.stop();
                    return false;
                }
            }

            if (window.UCOMMCHAT.visitorChat.chatOpened) {
                //If we are chatting or pending approval, don't do anything
                if (window.UCOMMCHAT.visitorChat.chatStatus == 'CHATTING' || window.UCOMMCHAT.visitorChat.chatStatus == 'OPERATOR_PENDING_APPROVAL') {
                    return false;
                }
            } else {
                //Otherwise start chat/email forms
                if (window.UCOMMCHAT.visitorChat.method == 'chat') {
                    window.UCOMMCHAT.visitorChat.startChat();
                } else if (window.UCOMMCHAT.visitorChat.method == 'chatbot') {
                    window.UCOMMCHAT.visitorChat.startChatBot();
                } else {
                    window.UCOMMCHAT.visitorChat.startEmail();
                }
        ...
        $('#visitorChat_logout').on('click keypress', $.proxy(function (event) {
            ...
            if (this.chatStatus == 'CHATTING' && !window.UCOMMCHAT.visitorChat.confirmClose()) {
                return false;
            }

            if (this.chatStatus == 'CHATTING') {
                window.UCOMMCHAT.visitorChat.changeConversationStatus("CLOSED");
                return false;
            }
        ...
        if (window.UCOMMCHAT.visitorChat.chatStatus == "LOGIN" || window.UCOMMCHAT.visitorChat.chatStatus == false) {
        ...
        $('#visitorChat_messageBox').one('keyup', function () {
            ...
            if (window.UNL.idm.getEmailAddress()) {
                $('#visitorChat_email').val(window.UNL.idm.getEmailAddress());
            }

            $('.visitorChat_info, #visitorChat_login_submit').slideDown('fast', function(){
                if (window.UCOMMCHAT.visitorChat.initialMessage && !$('#visitorChat_messageBox').is(':focus')) {
        ...
        $('#visitorChat_failedOptions_yes').click(function() {
            window.UCOMMCHAT.visitorChat.stop(function(){
                window.UCOMMCHAT.visitorChat.startEmail();
                if (window.UCOMMCHAT.visitorChat.initialMessage) {
                $('#visitorChat_messageBox').val(window.UCOMMCHAT.visitorChat.initialMessage);
                }

                $('#visitorChat_name').val(window.UCOMMCHAT.visitorChat.name);
                $('#visitorChat_email').val(window.UCOMMCHAT.visitorChat.email);
    ...
    handleIsTyping() {
        if (window.UCOMMCHAT.visitorChat.isTypingTimeout == false) {
            window.UCOMMCHAT.visitorChat.sendIsTypingStatus('YES');

            window.UCOMMCHAT.visitorChat.isTypingTimeout = setTimeout(function(){
                window.UCOMMCHAT.visitorChat.isTypingTimeout = false;
                window.UCOMMCHAT.visitorChat.sendIsTypingStatus('NO');

            }, 5000);
        }
    }

    sendIsTypingStatus(newStatus) {
        $.ajax({
            type:"POST",
    ...
    onConversationStatus_Emailed(data) {
        super.onConversationStatus_Emailed(data);
    ...
    onLogin() {
        ...
        window.UCOMMCHAT.visitorChat.deleteAnalyticsCookies();
        ...
        //Mark as started
        window.UNL.analytics.callTrackEvent('WDN Chat', 'Started');
    }
    ...
    onConversationStatus_Closed(data) {
        ...
        super.onConversationStatus_Closed(data);
    ...
    onConversationStatus_Chatting(data) {
        super.onConversationStatus_Chatting(data);
        ...
        if (is_typing) {
            $('#visitorChat_is_typing').text("The other party is typing.").show(500);
        } else {
    ...
    handleUserDataResponse(data) {
        ...
        super.handleUserDataResponse(data);
    ...
    onConversationStatus_OperatorLookupFailed(data) {
        ...
        window.UCOMMCHAT.visitorChat.operatorsAvailable = false;
    ...
    loadStyles() {
        ...
        $(window).on("load", function () {
            window.UCOMMCHAT.visitorChat.displaySiteAvailability();
        });
    ...
    displaySiteAvailability(available) {
        if (available == null) {
            available = window.UCOMMCHAT.visitorChat.operatorsAvailable;
        }
        ...
        if (available) {
            ...
            text = 'Live Chat';
            window.UCOMMCHAT.visitorChat.method = 'chat';
        } else if (this.isChatbotAvailable()) {
            $widget.addClass('online');
            $widget.removeClass('offline');
            text = 'Let\'s Chat';
            window.UCOMMCHAT.visitorChat.method = 'chatbot';
        } else {
            $widget.addClass('offline');
            $widget.removeClass('online');
            window.UCOMMCHAT.visitorChat.method = 'email';
        }
...
window.UCOMMCHAT.visitorChat = new VisitorChat_Client(serverURLPHP, refreshRatePHP);
